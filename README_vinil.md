...
## Interactive Dashboard & Recruiter Features

We have built a premium, feature-rich web interface in `app.py` for recruiters to analyze candidate rankings:

* **🏆 Shortlisted Leaderboard**: Interactive candidate list with match scores, and custom fact-based reasoning.
* **🚫 Disqualified Candidates Log**: Recruiter-transparency sub-tab displaying candidates who were knocked out by Stage 1 Hard Filters or Honeypot Traps, along with their specific diagnostic reasons (e.g. Unrelated Title, Location outside India, YOE < 3.0).
* **🔍 Profile Explorer**: Detailed inspector showing a candidate's background, Redrob trust and verification metrics, expected salary, work mode, and education history.
* **📊 Talent Analytics**: Demographics dashboards showing Score Distribution, Notice Period Breakdown, Location Analysis, and Experience (YOE) distributions via high-contrast horizontal charts.
* **⚖️ Side-by-Side Comparison**: Recruiter tool to select up to 3 candidates and evaluate their metrics and recent roles side-by-side.
...
|---|---|
| **Language** | Python 3.9+ (Fully compatible with Python 3.14 on Streamlit Cloud) |
| **Embeddings** | `sentence-transformers` (`BAAI/bge-small-en-v1.5`) |
| **Vector Space & Indexing** | `faiss-cpu` (IndexFlatIP) |
| **Data Processing & Dashboard** | `pandas`, `numpy`, `scikit-learn`, `streamlit` |
